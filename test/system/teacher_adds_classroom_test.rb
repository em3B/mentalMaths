...
class TeacherAddsClassroomTest < ApplicationSystemTestCase
  ...
  test "teacher can add a classroom from the teacher dashboard" do
    ...
    # Wait for dashboard to load (CI-safe)
    assert_current_path teacher_dashboard_path, ignore_query: true
    assert_text "Add a Classroom", wait: 10
    assert_selector 'input[placeholder="Classroom Name"]', wait: 10

    classroom_name = "Classroom #{SecureRandom.hex(3)}"
    find('input[placeholder="Classroom Name"]').set(classroom_name)
    click_button "Add"

    assert_text "Classroom created successfully.", wait: 10

    click_link "My Classrooms"
    assert_text classroom_name, wait: 10
  end
end
